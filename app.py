...
# Layout 3 Colonnes – Cyberpunk Light Mode (SAFE STRING VERSION)
...
if Path("style.css").exists():
    css_content = Path("style.css").read_text()
...
NODES = {
    ...
    "ganimed": {"label": "GANIMED // Nexus", "url": "https://ganimed.fr"},
}
...
# SESSION STATE
# ------------------------------------------------------------
st.session_state.setdefault("console", ["System initialized.", "Default node : cybermind", "Type 'help' for commands."])
...
def sandbox(node: str, mode: str):
    ...
    if "?embed=true" in url or is_streamlit_app(url):
        ...
        html_block = (
            "<div class='sandbox-frame mode-" + mode + "'>" +
            snapshot +
            "</div>"
        )
    ...
    components.html(html_iframe, height=0)
...
".join(st.session_state.console), height=360)
    ...
    if run and cmd:
        ...
        elif act == "theme": st.session_state.theme = pay
        elif act == "load":
            node, mode = pay
...
# ------------------------------------------------------------
with col_sandbox:
    st.markdown("### Sandbox — " + NODES[st.session_state.node]["label"])
...
with col_nodes:
    ...
    for key, meta in NODES.items():
        active = " node-active" if key == st.session
